...
int main() {
	...
	cout << "Do you want to play blackjack press Y or y for yes or press N or n for no ?" << endl;
	cin >> answer;
	while (answer == 'y' || answer == 'Y') {
		...
		while (options != 2 && playerscore != 21) {
			...
			if (options == 1) {
				...
				if (playerscore > 21) {
					cout << "GG you lose youre score is " << playerscore  << endl;
					playerLost = true;
					playerTurnend = true;
					break;
				}
		...
		if (playerHand2.at(0).at(0) == 'A' || playerHand2.at(1).at(0) == 'A') {
			...
			if (playerHand2.at(0).at(0) == 'A') {
				...
				playerscore2 += calculateScore(playerHand2.at(1));
			}
			else if (playerHand2.at(1).at(0) == 'A') {
				playerscore2 += calculateScore(playerHand2.at(0));
			}
		}
		else {
			for (int i = 0; i < playerHand2.size(); i++) {
		...
		for(int i = 0; i < playerHand1.size(); i++){
      	...
      	playerHand1.clear();
      	for(int i = 0; i < playerHand2.size(); i++){
      		gameDeck.push(playerHand2.at(i));
      	}
      	playerHand2.clear();

      	// if(playerHand1.empty() && playerHand2.empty()){
